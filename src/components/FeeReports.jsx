...
import { feeReportAPI } from '../utils/apiService';
...
const FeeReports = () => {
  ...
  const [feeData, setFeeData] = useState({ paid: [], due: [], defaulters: [] });
  ...
  const fetchFeeReports = async () => {
    try {
      setLoading(true);
      const res = await feeReportAPI.getAll();
      if (res.data && res.data.data) {
        setFeeData(res.data.data);
      }
    } catch (err) {
      console.error('Failed to fetch fee reports:', err);
  ...
  const displayData = feeData.paid.length > 0 ? feeData : staticFeeData;

  const handleExportReport = async () => {
    try {
      setLoading(true);
      const res = await feeReportAPI.download({ format: 'csv' });
  ...
  const pieChartOptions = useMemo(() => ({
    ...
    series: [{
      ...
      colorByPoint: true,
      data: [
        { name: 'Paid', y: displayData.paid.length, amount: displayData.paid.reduce((sum, item) => sum + item.amount, 0) },
        { name: 'Due', y: displayData.due.length, amount: displayData.due.reduce((sum, item) => sum + item.amount, 0) },
        { name: 'Defaulters', y: displayData.defaulters.length, amount: displayData.defaulters.reduce((sum, item) => sum + item.amount, 0) }
      ]
    }]
  }), [displayData]);
  ...
  const barChartOptions = useMemo(() => ({
    ...
    series: [{
      ...
      colorByPoint: true,
      data: [
        displayData.paid.reduce((sum, item) => sum + item.amount, 0),
        displayData.due.reduce((sum, item) => sum + item.amount, 0),
        displayData.defaulters.reduce((sum, item) => sum + item.amount, 0)
      ]
    }]
  }), [displayData]);
  ...
  const getCurrentData = () => {
    switch (activeTab) {
      case 'paid': return displayData.paid;
      case 'due': return displayData.due;
      case 'defaulters': return displayData.defaulters;
      default: return [];
    }
  };
  ...
  }, [activeTab, searchTerm, filterClass, displayData]);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                ₹{displayData.paid.reduce((sum, item) => sum + item.amount, 0).toLocaleString()}
              </p>
              <p className="text-sm text-gray-500">{displayData.paid.length} students</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-600">
                ₹{displayData.due.reduce((sum, item) => sum + item.amount, 0).toLocaleString()}
              </p>
              <p className="text-sm text-gray-500">{displayData.due.length} students</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600">
                ₹{displayData.defaulters.reduce((sum, item) => sum + item.amount, 0).toLocaleString()}
              </p>
              <p className="text-sm text-gray-500">{displayData.defaulters.length} students</p>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm">
        ...
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            {[
              { key: 'overview', label: 'Overview', count: displayData.paid.length + displayData.due.length + displayData.defaulters.length },
              { key: 'paid', label: 'Paid', count: displayData.paid.length },
              { key: 'due', label: 'Due', count: displayData.due.length },
              { key: 'defaulters', label: 'Defaulters', count: displayData.defaulters.length }
            ].map(tab => (
              <button
                key={tab.key}
          ...
          <>
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center space-x-2" onClick={handleExportReport}>
                  <MdDownload />
                  <span>Export</span>
                </button>
              </div>
