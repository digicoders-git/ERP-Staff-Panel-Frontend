...
import TransportAssign from './TransportAssign';
...
const Dashboard = ({ setIsLoggedIn }) => {
  ...
  const fetchStaffData = async () => {
    try {
      const res = await api.get('/api/staff-panel/auth/profile');
  ...
  const fetchDashboardStats = async () => {
    try {
      const res = await api.get('/api/staff-panel/dashboard/stats');
      if (res.data && res.data.stats) {
        setDashboardStats({
          newAdmissions: res.data.stats.newAdmissions || 0,
          totalAdmissions: res.data.stats.totalStudents || 0,
          feeCollections: res.data.stats.feeCollection || '₹0'
        });
  ...
  const sidebarItems = [
    ...
    { id: 'applications', name: 'Applications', icon: MdAssignment, path: '/applications' },
    { id: 'verification', name: 'Verification', icon: MdVerifiedUser, path: '/verification' },
    { id: 'enrollment', name: 'Enrollment', icon: MdAssignmentInd, path: '/enrollment' },
  ...
  const transportItems = [
    ...
    { id: 'transport-assignment', name: 'Transport Assignment', icon: MdSettings, path: '/transport-assignment' },
    { id: 'transport-allocation', name: 'Transport Allocation', icon: MdAssignmentInd, path: '/transport-allocation' },
    { id: 'transport-report', name: 'Transport Report', icon: MdBarChart, path: '/transport-report' }
  ];
  ...
  const studentItems = [
    { id: 'attendance-report', name: 'Student Attendance Record', icon: MdBarChart, path: '/attendance-report' },
    { id: 'student-leaves', name: 'Student Leave Management', icon: MdAssignment, path: '/student-leaves' }
  ];
  ...
  const teacherItems = [
    { id: 'teacher-management', name: 'Manage Teachers', icon: MdPeople, path: '/teacher-management' },
    { id: 'salary-management', name: 'Salary Management', icon: MdAttachMoney, path: '/salary-management' },
    { id: 'performance-evaluation', name: 'Performance Evaluation', icon: MdTrendingUp, path: '/performance-evaluation' }
  ];
  ...
  const libraryItems = [
    { id: 'library-dashboard', name: 'Dashboard', icon: MdDashboard, path: '/library-dashboard' },
  ...
  const stats = [
    { title: 'New Admissions', count: dashboardStats.newAdmissions, icon: MdAppRegistration, color: 'bg-blue-500' },
    { title: 'Total Admissions', count: dashboardStats.totalAdmissions, icon: MdPersonAdd, color: 'bg-purple-500' },
    { title: 'Fee Collections', count: dashboardStats.feeCollections, icon: MdPayment, color: 'bg-emerald-500' }
  ];
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <div className="text-right hidden sm:block">
                <p className="text-sm font-black text-slate-800 group-hover:text-blue-600 transition-colors">{staffData?.name || 'Staff'}</p>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-wider">{staffData?.designation || 'Staff Member'}</p>
              </div>
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50/50 scroll-smooth">
          <div className="max-w-[1600px] mx-auto p-6 md:p-10">
            <Routes>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        {recentActivities.length > 0 ? (
                          recentActivities.map((item, i) => (
                            <div key={i} className="flex items-center gap-5 group">
                              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center bg-blue-50 text-blue-600 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-sm`}>
                                <MdAppRegistration size={22} />
                              </div>
                              <div className="flex-1">
                                <p className="text-sm font-bold text-slate-800 tracking-tight">{item.activity}</p>
                          ...
                          <div className="text-center py-10">
                            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No recent activities</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100">
                      ...
                      <div className="space-y-5">
                        {[
                          { stage: 'Confirmed', info: `${dashboardStats.totalAdmissions} Active Students`, val: 'Live', clr: 'bg-emerald-600' },
                          { stage: 'New Admissions', info: `${dashboardStats.newAdmissions} Last 30 Days`, val: 'Pending', clr: 'bg-blue-600' },
                          { stage: 'Financials', info: `Monthly ${dashboardStats.feeCollections}`, val: 'Collected', clr: 'bg-indigo-600' }
                        ].map((item, i) => (
              ...
              <Route path="/transport-assignment" element={<TransportAssign />} />
              ...
              <Route path="/attendance-process" element={<AttendanceRegistry />} />

              <Route path="/attendance-report" element={<AttendanceReports />} />
              <Route path="/student-leaves" element={<StudentLeaveManagement />} />
              <Route path="/staff-attendance-report" element={<StaffAttendance />} />
