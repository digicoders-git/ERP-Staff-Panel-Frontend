...
import { FaChartBar, FaBuilding, FaBed, FaUsers, FaUserTie, FaUser, FaHome, FaSpinner } from 'react-icons/fa';
...
export default function HostelReports() {
  ...
  const getHostelWiseReport = () => {
    return hostels.map(hostel => {
      const hostelRooms = rooms.filter(room => (room.hostel?._id || room.hostel) === hostel._id);
      const hostelAllocations = allocations.filter(allocation => (allocation.hostel?._id || allocation.hostel) === hostel._id);
      const hostelWardens = wardens.filter(warden => (warden.assignedHostel?._id || warden.assignedHostel) === hostel._id);
  ...
  const getRoomTypeReport = () => {
    return roomTypes.map(roomType => {
      const typeRooms = rooms.filter(room => (room.roomType?._id || room.roomType) === roomType._id);
      const typeAllocations = allocations.filter(allocation => {
        const room = rooms.find(r => r.roomNo === allocation.roomNo && (r.hostel?._id || r.hostel) === (allocation.hostel?._id || allocation.hostel));
        return room && (room.roomType?._id || room.roomType) === roomType._id;
      });
      ...
      return {
        ...
        totalCapacity: typeRooms.reduce((sum, r) => sum + parseInt(r.capacity || 0), 0),
        occupied: typeAllocations.length,
        available: typeRooms.reduce((sum, r) => sum + parseInt(r.capacity || 0), 0) - typeAllocations.length,
  ...
  const getStudentListReport = () => {
    return allocations.map(allocation => {
      ...
      return {
        ...allocation,
        hostelDisplay: allocation.hostel?.hostelName || 'N/A',
        roomTypeDisplay: room?.roomType?.roomTypeName || 'N/A'
      };
    });
  };
  ...
  const getWardenReport = () => {
    return wardens.map(warden => {
      const assignedStudents = allocations.filter(allocation => 
        (warden.assignedHostel?._id || warden.assignedHostel) === (allocation.hostel?._id || allocation.hostel)
      ).length;
      
      return {
        ...warden,
        assignedStudents,
        hostelName: warden.assignedHostel?.hostelName || 'Unassigned',
            ...
            <table className="w-full">
              ...
              <tbody>
                {hostelData.map((hostel) => (
                  <tr key={hostel.hostelName} className={`border-b hover:bg-gray-50 ${hostelData.indexOf(hostel) % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
            ...
            <table className="w-full">
              ...
              <tbody>
                {roomTypeData.map((roomType) => (
                  <tr key={roomType.roomTypeName} className={`border-b hover:bg-gray-50 ${roomTypeData.indexOf(roomType) % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
            ...
            <table className="w-full">
              ...
              <tbody>
                {studentData.map((student) => (
                  <tr key={student._id} className={`border-b hover:bg-gray-50 ${studentData.indexOf(student) % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                    <td className="px-4 py-3 font-bold">{student.studentId}</td>
                    <td className="px-4 py-3">{student.studentName}</td>
                    <td className="px-4 py-3">{student.hostelDisplay}</td>
                    <td className="px-4 py-3">{student.roomNo}</td>
                    <td className="px-4 py-3">{student.roomTypeDisplay}</td>
                    <td className="px-4 py-3">{student.joiningDate?.split('T')[0] || 'N/A'}</td>
            ...
            <table className="w-full">
              ...
              <tbody>
                {wardenData.map((warden) => (
                  <tr key={warden._id} className={`border-b hover:bg-gray-50 ${wardenData.indexOf(warden) % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">{warden.mobileNumber || warden.email || 'N/A'}</td>
                    <td className="px-4 py-3 capitalize">{warden.gender}</td>
                    <td className="px-4 py-3">
                      <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-lg text-xs font-bold">
                        {warden.hostelName}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center font-bold">{warden.hostelCount}</td>
                    <td className="px-4 py-3 text-center text-indigo-600 font-black">{warden.assignedStudents}</td>
                    <td className="px-4 py-3 capitalize">{warden.shift}</td>
                    <td className="px-4 py-3">
        ...
        <div className="p-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Aggregating Residential Analytics...</p>
            </div>
          ) : renderReport()}
        </div>
      </div>
