...
import { eLearningAPI } from '../utils/apiService';
...
const QuizManager = () => {
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      // If you want to load existing quizzes, handle here
    } catch (err) {
  ...
  const saveQuiz = async () => {
    if (quizData.title && quizData.questions.length > 0) {
      try {
        setLoading(true);
        await eLearningAPI.createQuiz(quizData);
      ...
      toast.warning('Please fill in all required fields and add at least one question.');
    }
  };
